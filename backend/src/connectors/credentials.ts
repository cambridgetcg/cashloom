...
// are nameable; everything else is unreachable by construction. Enforced both
// at input validation (account.validator) and here at resolution, so no
// write path — present or future — can widen the namespace.
export const CREDENTIAL_REF_PATTERN = /^(STRIPE|GOCARDLESS)_[A-Z0-9_]+$/;
...
export const resolveCredentialRef = (ref: string): string => {
  ...
  if (!isAllowedCredentialRef(ref)) {
    throw new InternalServerException(
      `Refusing to resolve credentialRef "${ref}": only connector-credential environment variables (STRIPE_* / GOCARDLESS_*) can be referenced`
    );
  }
