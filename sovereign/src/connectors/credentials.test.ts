...
// GOCARDLESS_* / ALCHEMY_* / AGENTTOOL_*) — anything else is refused before
...
describe("the closed credentialRef namespace", () => {
  ...
  it.each([
    ...
    "alchemy_api_key",
    "AGENTTOOLX_FOO", // same prefix-not-substring rule for the agenttool namespace
    "agenttool_api_key",
    "AGENTTOOL_BASE_URL", // deploy config, not a credential — excluded by the lookahead
  ...
  it("resolves refs inside the namespace", () => {
    ...
    vi.stubEnv("AGENTTOOL_API_KEY", "FAKE-at-key");
    ...
    expect(resolveCredentialRef("AGENTTOOL_API_KEY")).toBe("FAKE-at-key");
  });
  ...
  it("isAllowedCredentialRef matches only the connector namespace", () => {
    ...
    expect(isAllowedCredentialRef("AGENTTOOL_API_KEY")).toBe(true);
    ...
    expect(isAllowedCredentialRef("AGENTTOOL_")).toBe(false);
    expect(isAllowedCredentialRef("AGENTTOOLX_FOO")).toBe(false);
    // Base-URL overrides are deploy config, never nameable as credentials.
    expect(isAllowedCredentialRef("AGENTTOOL_BASE_URL")).toBe(false);
  });
});
