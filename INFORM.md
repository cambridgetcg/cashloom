...
3. **Covenant draft (hosted only)**: `rights-adoption.json` is a CashLoom-scoped ledger with all 10 rights / 38 requirement_results / 5 protective limits enumerated and **honest gaps showing**; it is `status:"draft"` with `authority_state:"unverified"`, distinct from Surface conformance, and served at `/.well-known/xenia-rights.json`. Its schema, Covenant, and matching beta.4 cross-document validator are immutable-version pinned and the exact validator runs in the test gate. **Never self-activate it**: activation would still require separately verified release evidence, representative authority, and an explicit operator ceremony.
...
3. **Covenant activation posture** — stays `draft`; immutable source pins remove locator drift but do not establish representative authority, release evidence, or implementation. Activation remains a speaker-authority ceremony, yours.
